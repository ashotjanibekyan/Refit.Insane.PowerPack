...
public class RefitRestServiceCacheDecoratorTests
{
    ...
    public void Setup()
    {
        ...
        _mockedRestService.Setup(x => x.Execute<ICacheRestMockApi, IEnumerable<string>>(
            It.IsAny<Expression<Func<ICacheRestMockApi, Task<IEnumerable<string>>>>>(), It.IsAny<RefitCacheBehaviour>()
        )).ReturnsAsync(new Response<IEnumerable<string>>(new List<string>()));

        _persistedCacheMock = new Mock<IPersistedCache>();
        _persistedCacheMock.Setup(x => x.Get<IEnumerable<string>>(It.IsAny<RefitCacheLocation>(), It.IsAny<string>()))!.ReturnsAsync(
            default(IEnumerable<string>));
        _persistedCacheMock.Setup(x =>
            x.Save<IEnumerable<string>>(It.IsAny<RefitCacheLocation>(), It.IsAny<string>(), It.IsAny<IEnumerable<string>>(), It.IsAny<TimeSpan?>()));

        _sut = new RefitRestServiceCachingDecorator(_mockedRestService.Object, _persistedCacheMock.Object, new RefitCacheController());
    }

    private RefitRestServiceCachingDecorator _sut;
    private Mock<IRestService> _mockedRestService;
    private Mock<IPersistedCache> _persistedCacheMock;
    ...
    public async Task Execute_RestInterfaceDoesHaveCacheAttribute_WithDefaultNotForcedFlag_PersistedCacheIsCalledWithSaveOnce()
    {
        ...
        await systemUnderTest.Execute<ICacheRestMockApi, IEnumerable<string>>(api => api.GetItems("test"));

        persistedCacheMock.Verify(
            x => x.Save(It.IsAny<RefitCacheLocation>(), It.IsAny<string>(), It.IsAny<IEnumerable<string>>(), It.IsAny<TimeSpan?>()), Times.Once);
    ...
    public async Task Execute_RestInterfaceDoesHaveCacheAttribute_WithForceExecuteFlag_PersistedCacheIsCalledWithSaveTwice()
    {
        ...
        await systemUnderTest.Execute<ICacheRestMockApi, IEnumerable<string>>(api => api.GetItems("test"),
            RefitCacheBehaviour.ForceUpdateWithoutFallbackToCache);
        ...
        persistedCacheMock.Verify(
            x => x.Save(It.IsAny<RefitCacheLocation>(), It.IsAny<string>(), It.IsAny<IEnumerable<string>>(), It.IsAny<TimeSpan?>()),
            Times.Exactly(2));
    }

    [Test]
    public async Task
        Execute_RestInterfaceDoesHaveCacheAttribute_WhenCacheHasItem_WhenPersistedStorageReturnsTimeSpanLessThan24Hours_PersistedCacheSaveIsNotCalled()
    {
        var persistedCacheMock = new Mock<IPersistedCache>();

        persistedCacheMock.Setup(x => x.Get<IEnumerable<string>>(It.IsAny<RefitCacheLocation>(), It.IsAny<string>()))!.ReturnsAsync(new List<string>
            { "test cache" });
        ...
        await systemUnderTest.Execute<ICacheRestMockApi, IEnumerable<string>>(api => api.GetItems("test"),
            timeSpan => timeSpan.HasValue && timeSpan.Value.TotalHours >= 24
                ? RefitCacheBehaviour.ForceUpdateWithoutFallbackToCache
                : RefitCacheBehaviour.Default);

        persistedCacheMock.Verify(
            x => x.Save(It.IsAny<RefitCacheLocation>(), It.IsAny<string>(), It.IsAny<IEnumerable<string>>(), It.IsAny<TimeSpan?>()), Times.Never);
    }

    [Test]
    public async Task
        Execute_RestInterfaceDoesHaveCacheAttribute_WhenCacheHasItem_WhenPersistedStorageReturnsTimeSpanHigherThan24Hours_PersistedCacheSaveIsNotCalled()
    {
        var persistedCacheMock = new Mock<IPersistedCache>();
